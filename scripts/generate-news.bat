...
claude -p "다음 작업을 수행해줘: 1) 아래 RSS 피드에서 최신 프리다이빙/다이빙 뉴스를 수집해: https://www.deeperblue.com/feed/ 와 https://news.google.com/rss/search?q=freediving&hl=en&gl=US&ceid=US:en 와 https://news.google.com/rss/search?q=프리다이빙&hl=ko&gl=KR&ceid=KR:ko 2) 수집한 뉴스 중 가장 중요한 3개를 선별해. 3) 각 뉴스에 대해 content/articles/ 아래 오늘 날짜 폴더에 JSON 파일을 생성해. 기존 기사 파일(content/articles/2026/04/15/molchanov-ice-freediving-record-baikal.json)과 동일한 JSON 형식으로. slug, publishedAt, updatedAt, status는 published, category, tags, en과 ko 각각 title/summary/body(HTML)/metaDescription, imageUrl은 Unsplash URL, sourceUrls는 원본 URL. 4) 본문은 각 언어로 300-500단어 HTML로 작성. 원본을 복사하지 말고 새로운 기사 작성. 5) 이미 같은 slug 파일이 있으면 건너뛰어." --allowedTools "Bash,Read,Write,Edit" --max-turns 20
